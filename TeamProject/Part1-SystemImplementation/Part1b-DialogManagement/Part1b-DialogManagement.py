...
def remove_duplicates_from_keys(my_dict):
    for key in my_dict:
        seen_values = set()
        new_values = []
...
def keyword_matching(utterance):
    ...
    #check if any misspelled preferences are possibly included through simple speech patterns:
    #{variable} food       (food, price)
    #{variable} restaurant (food, price)
    #{variable} place      (food, price)
    #{variable} priced     (price)
    #{variable} area       (area)
    #{variable} part       (area)
    ...
    for i, word in enumerate(words):
        ...
        if word == "food" and i > 0:
            keywords['food'].append(words[i-1])
            keywords['pricerange'].append(words[i-1])

        if word == "restaurant" and i > 0:
            keywords['food'].append(words[i-1])
            keywords['pricerange'].append(words[i-1])

        if word == "place" and i > 0:
            keywords['food'].append(words[i-1])
            keywords['pricerange'].append(words[i-1])

        if word == "priced" and i > 0:
            keywords['pricerange'].append(words[i-1])

        if word == "area" and i > 0:
            keywords['area'].append(words[i-1])

        if word == "part" and i > 0:
            keywords['area'].append(words[i-1])

        if i < len(words) - 2:
            if word == "in":
                if words[i+1] == "the":
                    keywords['area'].append(words[i+2])

    #remove duplicates for each key
    keywords = remove_duplicates_from_keys(keywords)

    #check if any preferences are stated (correctly spelled) through string matching, overwrites all
    #previously defined values for that key if so
    for key in domain_terms_dict:
        for value in domain_terms_dict[key]:
...
def check_misspelling_or_preferences(cur_utterance):
    ...
    for key in domain_terms_dict:
        for value in domain_terms_dict[key]:
            preferences, misspelling = levenshtein_distance(value, key, domain_terms_dict, preferences)
...
def main():
    ...
    while True:
        ...
        check_misspelling_or_preferences(utterance)
